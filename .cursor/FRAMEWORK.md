...
Phase 10 (Design System Evolution — web systems only)
/08d + text --> /08e
(doc only)       (code only, auto)

Phase 11 (Frontend Improvement — web systems only)
/09 + text  (standalone — analyze → plan → implement)
```
...
| 10. Evolve Design System *(web only)* | Run `/08d` + write change request | Design system change intent is human judgment |
| 10b. Implement DS Evolution *(web only)* | _(automatic after `/08d`)_ Run `/08e` | Zero input — reads DS Impact Plan from disk |
| 11. Improve Frontend *(web only)* | Run `/09` + describe what to improve | Improvement intent and scope are human judgment |
...
│   │   └── design-system/            <-- Design system rules (2 files, conditional)
│   ├── templates/
...
│   │   └── design-system/            <-- Design system templates (10+ files, conditional)
...
│   │   ├── 08d-evolve-design-system.md → /08d  (web only)
│   │   ├── 08e-implement-design-system-evolution.md → /08e  (web only)
│   │   └── 09-improve-frontend.md    → /09  (web only)
...
### Phase 10: Evolve Design System *(web systems only)*

**What it does**: Updates design system documentation — tokens, components, visual direction, pillar profiles, page archetypes, interaction patterns. Produces a DS Impact Plan that drives Phase 10b.

**Command**: `/08d`

**Agent**: UX Architect (with design system governance mode)

**Skill**: Design System Compliance Audit (pre-change + post-change)

**User input**: Change request — what to add, change, or remove in the design system.

**Output**: Updated design system documents, ADR (if L2+), updated LLM-QUICKREF.md, DS Impact Plan at `docs/design-system/evolution/`

**Next**: Run `/08e` to implement the code changes defined by the DS Impact Plan.

---

### Phase 10b: Implement Design System Evolution *(web systems only)*

**What it does**: Implements frontend code changes defined by the DS Impact Plan produced in Phase 10. Updates token references, adds or modifies components, updates patterns throughout the codebase.

**Command**: `/08e`

**Agent**: Frontend Engineer

**Skill**: Design System Compliance Audit (verification)

**User input**: None — reads DS Impact Plan from disk automatically.

**Output**: Updated frontend code matching the evolved design system; compliance audit report

---

### Phase 11: Improve Frontend *(web systems only)*

**What it does**: Analyzes an existing frontend element (page, component, layout, or flow), identifies design system violations and UX improvement opportunities, proposes an Improvement Brief for user confirmation, then implements. Standalone — requires no prior design system or architecture evolution.

**Command**: `/09`

**Agent**: Frontend Engineer (improvement analysis mode)

**Skill**: Design System Compliance Audit (analysis + verification)

**User input**: Description of what to improve — specific page, component, flow, or layout.

**Output**: Improvement Brief, updated frontend code, compliance audit report with before/after comparison
...
| `/08d` | 10 | Evolve design system documentation *(web only)* | ✏️ Change request (text after command) |
| `/08e` | 10b | Implement design system evolution in frontend code *(web only)* | None (reads DS Impact Plan) |
| `/09` | 11 | Improve existing frontend element *(web only)* | ✏️ Description of what to improve |
...
| File | Key Rules | Purpose |
|------|-----------|---------|
| `design-system.mdc` | DS-001 to DS-022 | Design consistency standards, component naming, token conventions, pillar system, page archetypes (web systems only) |
| `frontend-coding.mdc` | FE-001 to FE-022 | Frontend code standards: token usage in code, component structure, interactive states, responsive patterns, anti-patterns (web systems only) |
...
| AGT-FE | `agents/frontend-engineer.md` | Phase 10b, 11 | Design-system-faithful implementation, UX quality analysis | SKL-DSCOMP |
...
| SKL-DSCOMP | `skills/design-system-compliance-audit.md` | Design System Governance | `/08d`, `/08e`, `/09`, `/06` |
...
- **Design system evolution**: use `/08d` (evolve DS docs) → `/08e` (implement in code) when the design system itself needs to change (new tokens, new components, changed visual direction)
- **Frontend improvement**: use `/09` for iterative UX and visual refinements on existing pages and components — no design system change required
- **Frontend coding rule**: `.cursor/rules/design-system/frontend-coding.mdc` governs all frontend code — token usage, component structure, interactive states, responsive behavior
...
| `/08d` | 10. Evolve DS *(web only)* | ✏️ Change request | Updated design system docs + DS Impact Plan |
| `/08e` | 10b. Implement DS Evolution *(web only)* | None | Frontend code updated per DS Impact Plan |
| `/09` | 11. Improve Frontend *(web only)* | ✏️ What to improve | Improvement Brief + frontend code improved |
...
```
 ...
 │                                    │──> /05  (auto implement)         │
 │                                    │                                  │
 │  [10] /08d + change request ───────┤──> /08e (auto frontend code)    │
 │        (web only)                  │    (web only)                    │
 │                                    │                                  │
 │  [11] /09 + what to improve ───────┘  (web only, standalone)         │
 │        (web only)                                                      │
 │                                                                        │
 └────────────────────────────────────┴────────────────────────────────┘
```
