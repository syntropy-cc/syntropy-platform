# Implement Design System Evolution (Prompt 08-E)

Use this prompt to implement frontend code changes after a design system evolution. This is **Phase 10b** of the Vision-to-System Framework lifecycle. It reads the DS Impact Plan produced by `/08d` automatically — no copy-paste needed.

**This prompt is code-only.** It translates the DS Impact Plan into frontend code changes. It does not modify design system documentation.

**Prerequisite**: Complete `/08d` first. The DS Impact Plan at `docs/design-system/evolution/` is this prompt's input. Running this prompt without a DS Impact Plan will result in a halt.

**Analogy in the framework**: This prompt is to design system evolution what `/05` is to architecture implementation. The documentation has been evolved; this prompt implements the code consequence.
...
1. Complete **`/08d`** — the DS Impact Plan is saved automatically at `docs/design-system/evolution/`
2. Open a **new** Cursor conversation
3. Copy everything from the "Prompt" section above into the conversation
4. The AI will read the DS Impact Plan automatically — no copy-paste needed
5. Review the implementation plan (file manifest, token changes, component updates); confirm when satisfied
6. The AI implements the changes and runs the compliance audit
7. After completing, review the compliance audit report — address any failures before considering the evolution complete

**When to use this prompt vs. `/09`**:
- Use `/08e` when you have a DS Impact Plan from `/08d` — this prompt implements that specific, documented change
- Use `/09` when you want to improve frontend code without a prior design system change
