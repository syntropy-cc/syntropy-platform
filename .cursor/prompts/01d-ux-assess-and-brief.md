# UX and Interaction Design: Assess and Brief (Prompt 01-D)

Use this prompt to evaluate the system's user-facing interface requirements and produce a confirmed **UX Brief** before any UX document is created. This is **Step 1 of 2** in the UX and interaction design phase (Phase 2b) of the Vision-to-System Framework.

**This prompt is conditional**: use it only if the system exposes any user-facing interface (CLI, Web, Dashboard, Mobile). If `docs/context/generation-summary.md` shows no user-facing interfaces, skip Phase 2b and proceed directly to Prompt 03.

**This prompt reads the Generation Summary from `docs/context/generation-summary.md` automatically.** No copy-paste from the previous session is needed. If that file is missing, the prompt will halt.

**This prompt runs entirely in Cursor's Plan mode.** It produces no UX files. Its outputs are: the confirmed UX Brief (written to `docs/context/ux-brief.md`) and an updated execution state.

After the user confirms the UX Brief, proceed to **Prompt 01-E** (`.cursor/prompts/01e-ux-generate-and-validate.md`).
...
1. Complete **Prompt 01-C** (the Generation Summary is saved automatically)
2. Confirm the system has a user-facing interface (check `docs/context/generation-summary.md`); if not, skip to Prompt 03
3. Open a **new** Cursor conversation in **Plan mode** — no pasting of the Generation Summary needed
4. Copy everything from the "Prompt" section above into the conversation
5. Send — the AI will read the Generation Summary from disk, identify interface types, apply the DS-001 gate, and produce the UX Brief
6. Review the UX Brief; confirm when satisfied — it is saved automatically
7. Open a new conversation and run **Prompt 01-E** — no copy-paste needed
