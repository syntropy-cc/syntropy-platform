# Visual Direction and Image Prompts (Prompt 01-F)

Use this prompt to translate the completed design system into an aesthetic narrative and a library of AI image generation prompts. This is **Step 3 of 3** in the UX and interaction design phase (Phase 2b) of the Vision-to-System Framework — and it is **conditional**: run it only if Prompt 01-E created a design system (i.e., the DS-001 gate in the UX Brief was "Required").

**This prompt reads the UX Generation Summary from `docs/context/ux-generation-summary.md` automatically.** No copy-paste from the previous session is needed. If that file is missing, the prompt will halt.

After completing this prompt, Phase 2b is complete. Proceed to **Prompt 03** (`.cursor/prompts/03-generate-implementation-docs.md`).
...
1. Complete **Prompt 01-E** (the UX Generation Summary is saved automatically)
2. Confirm the DS-001 gate result was "Required" — if not, skip to Prompt 03
3. Open a **new** Cursor conversation — no pasting of the UX Generation Summary needed
4. Copy everything from the "Prompt" section above into the conversation
5. **Start in Plan mode**: the AI will read the context file and design system, produce the Visual Direction Brief, and await confirmation
6. Review the Visual Direction Brief — especially the aesthetic archetype and anti-patterns; confirm when satisfied
7. Switch to Agent mode to execute
8. After completing, proceed to **Prompt 03** — no copy-paste needed
