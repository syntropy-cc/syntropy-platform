# Quality Audit (Prompt 06)

Use this prompt after all implementation stages are complete (Phase 6 done) and before generating user documentation (Phase 7). This is **Phase 6b** of the Vision-to-System Framework.

The Quality Audit is a mandatory quality gate. Critical failures from this audit must be resolved before proceeding to Phase 7.
...
1. Ensure all implementation stages are complete (Phase 6 fully done)
2. Open a **new** Cursor conversation
3. Copy everything from the "Prompt" section above into the conversation
4. Send — the AI will run both assessments and produce the combined Quality Audit Report
5. If verdict is FAIL: resolve the blockers, then open a new conversation with this prompt again to re-audit
6. If verdict is PASS or PASS WITH WARNINGS: proceed to **Prompt 07** (`.cursor/prompts/07-generate-user-documentation.md`)
